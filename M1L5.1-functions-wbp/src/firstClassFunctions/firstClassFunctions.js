...
function formatProducts(carousel, grid, sidebar) {
  const gridProductsReformatted = grid.map(reausableFunction)
  const sidebarProductsReformatted = sidebar.map(reausableFunction)
  const carouselProductsReformatted = carousel.map(reausableFunction)
  return [
    ...
    function reausableFunction (product){
