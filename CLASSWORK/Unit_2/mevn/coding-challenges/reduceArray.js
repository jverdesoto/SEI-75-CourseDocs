...
invoke the callback function (2nd arg), passing to it three arguments: (1) the "accumulator", which is the value returned by 
...
//=> {"Yes": 2, "No": 1, "Maybe": 1}
-----------------------------------------------------------------*/
module.exports = function reduceArray(arr, callbackFn, acc) {
    for (let i = 0; i < arr.length; i++) {
        acc = callbackFn(acc, arr[i], i)
    }
    return acc
}