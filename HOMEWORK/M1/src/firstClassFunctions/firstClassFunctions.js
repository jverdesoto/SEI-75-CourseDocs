...
function formatProducts(carousel, grid, sidebar) {
  ...
  function singleProduct(product) {
      ...
      let ammendedProduct = capitalizedProductWords.join(" ");
      return ammendedProduct;
  }
  const carouselProductsReformatted = carousel.map(singleProduct);
  const gridProductsReformatted = grid.map(singleProduct);
  const sidebarProductsReformatted = sidebar.map(singleProduct)
