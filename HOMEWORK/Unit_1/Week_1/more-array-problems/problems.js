...
function multiplyBy10(array) {
    const multiplidNumbers = [];
    array.map((number)=> {
      multiplidNumbers.push(Math.abs(number) * 10);
    });
    return multiplidNumbers;
}
...
function findIndex(val, array) {
    let index = Array.from(array).indexOf(val);
...
function sumOfArray(array) {
   return array.reduce((total, item) => total + parseInt(item), 0); 
}
