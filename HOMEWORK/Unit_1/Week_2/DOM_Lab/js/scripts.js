...
document.getElementById('title-container').prepend(title);
...
data.forEach(post => {
    ...
    title.innerHTML = `${post.title} <span>${new Date(post.createdAt).toLocaleDateString()}</span>`;
    ...
    authorAvatar.src = post.authorAvatar;
...
fetch('https://cataas.com/cat?json=true')
    ...
    .then(data => {
        ...
        catImg.src = `https://cataas.com${data.url}`;
        catImg.className = 'cat-circle';
        document.getElementById('cat-container').appendChild(catImg);
    });