...
function formatProducts(carousel, grid, sidebar) {
  const carouselProductsReformatted = generalReformat(carouselProducts);
  const gridProductsReformatted = generalReformat(gridProducts);
  const sidebarProductsReformatted = generalReformat(sidebarProducts);
    function generalReformat(item){
        const name = item.map((product) => {
