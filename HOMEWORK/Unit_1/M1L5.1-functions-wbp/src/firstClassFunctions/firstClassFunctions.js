...
function formatProducts(carousel, grid, sidebar) {
  const carouselProductsReformatted = carousel.map(productsReformat);
  const gridProductsReformatted = grid.map(productsReformat);
  const sidebarProductsReformatted = sidebar.map(productsReformat);
  return [
...
function productsReformat (product) {
