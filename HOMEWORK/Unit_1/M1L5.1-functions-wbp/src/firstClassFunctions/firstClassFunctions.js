...
function reformatter (product) {
  const productsReformatted = carouselProducts.map((product) => {
...
function formatProducts(carousel, grid, sidebar) {
  const carouselProductsReformatted = carousel.map(reformatter);
  const gridProductsReformatted = grid.map(reformatter);
  const sidebarProductsReformatted = sidebar.map(reformatter);
  
  return carouselProductsReformatted
  return gridProductsReformatted
  return sidebarProductsReformatted
}
