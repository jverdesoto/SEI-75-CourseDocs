...
function formatProducts(carousel, grid, sidebar) {
  const carouselProductsReformatted = carousel.map(mapFunction);
  const gridProductsReformatted = grid.map(mapFunction);
  const sidebarProductsReformatted = sidebar.map(mapFunction);
