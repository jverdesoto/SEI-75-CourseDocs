...
function formatTitle(title) {
	...
	const capitalised = titleArr.map((word) => (word.replace(word.charAt(0), word.charAt(0).toUpperCase())))
	return capitalised.join(" ")
}

// Populate cards function
function populateCard(post, cardId) {
    ...
    postTitle.innerHTML = formatTitle(post.title)
    postDate.innerHTML = formatDate(post.createdAt)
    postBody.innerHTML = post.body
    postAuthorAvatar.innerHTML = `<img src="${post.authorAvatar}">`
    postAuthor.innerHTML = post.authorName
}

// Loops over the data publishing the cards
for (post of publishPosts) {
    cardCount++
	populateCard(post, `card${cardCount}`)
	if (cardCount === postsToShow) break
}
