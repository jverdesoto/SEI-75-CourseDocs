...
function numberOfVowels(string) {
  ...
  const charArray = lowerCaseString.split("")
  const vowelArray = charArray.filter((item) => (item === "a" || item === "e" || item === "i" || item === "o" || item === "u"))
...
function titleCase(string) {
  ...
  const capitalisedArray = wordArray.map((item) => (item.replace(item.charAt(0), item.charAt(0).toUpperCase())))
