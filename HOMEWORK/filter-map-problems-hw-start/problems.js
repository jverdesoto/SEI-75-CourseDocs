...
function convertTemps(array) {
  return array.map(item => (item - 32) * 5/9)
}
...
// ? eg: containsFalsey([100, {}, [], 'Mike']) => [], containsFalsey([100, {}, NaN, 'Mike', '', null]) => [NaN, '', null]
...
function numberOfVowels(string) {
  ...
  const vowelCount = splitArr.map((word) => word.match(/[aeiou]/gi).length)
  return vowelCount.reduce((accumulator, currentValue) => {return accumulator + currentValue} ,0)
}
...
function titleCase(string) {
  ...
  const capitalizeYeah = arrOfWords.map((word) => {
    const firstLetterCapt = word.charAt(0).toUpperCase()
    const restOfWord = word.slice(1).toLowerCase()
    return firstLetterCapt + restOfWord
  })
  return capitalizeYeah.join(' ')
}
