...
6. Programmatic Routing Using the `useNavigate` Hook

## 1. Setup

The code for this lesson begins right where we left off in the _MERN-Stack Shopping Cart - Part 1_ lesson.

- Move into the `sei-cafe` project folder:
    ```
    cd ~/code/sei-cafe
    ```
...
<code>git reset --hard origin/sync-cafe-10-shop-2-starter</code>
...
<details>
<summary>
❓ Why is the <code>isPaid</code> prop there?  In other words, why would a line item need to know if the order is paid or not? Browse to <a href="https://sei-cafe.herokuapp.com/orders/new">the deployed app</a> for a hint - be sure to have at least one item in your cart.
</summary>
<hr>

<code>&lt;LineItem&gt;</code> should not allow the quantity to be changed if the order is already paid - so it should not render the <code>[-]</code> and <code>[+]</code> buttons.
...
#### 👉 You Do - `handleAddToOrder` (4 minutes)
...
#### 👉 You Do - Code `handleAddToOrder` (2 minutes)
...
<code>git reset --hard origin/sync-cafe-11-shop-2-add-item</code>
...
<code>git reset --hard origin/sync-cafe-12-shop-2-qty</code>
...
<code>git reset --hard origin/sync-cafe-13-shop-2-finish</code>
